...
TEST_CASE("GreensTensorPlateVacuum calculate_tensor function returns the sum "
  ...
  auto GTPlateVacuum = std::make_shared<GreensTensorPlateVacuum>(v, beta, za, refl, delta_cut, rel_err);
  auto GTPlate = std::make_shared<GreensTensorPlate>(v, beta, za, refl, delta_cut, rel_err);
  auto GTVacuum = std::make_shared<GreensTensorVacuum>(v, beta, rel_err(0));
