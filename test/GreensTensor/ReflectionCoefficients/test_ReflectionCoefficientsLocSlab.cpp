...
TEST_CASE("Constructors of the slab reflection coefficients work properly", "[ReflectionCoefficientsLocSlab]"){
  ...
  SECTION("Constructor with ini file works") {
    ...
    Permittivity *perm = new PermittivityDrude("../data/test_files/GreensTensorSlab.ini");

    REQUIRE(RefC.get_epsilon(omega).real() == perm->epsilon(omega).real());
    REQUIRE(RefC.get_thickness() == 0.05);
  }
};
TEST_CASE("For slab thickness*kappa >> 1, the bulk coefficient is retrieved", "[ReflectionCoefficientsSlab]"){
...
TEST_CASE("r_p and r_s of the slab configuration obey the crossing relation", "[ReflectionCoefficients]"){
