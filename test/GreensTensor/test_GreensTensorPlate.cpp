...
TEST_CASE("The operations calculate_tensor and the integrand_2d_k coincide",
  ...
  PermittivityDrude perm(gamma, omega_p);
    ReflectionCoefficientsLocBulk refl(&perm); 
  GreensTensorPlate Greens(v, za, NAN, &refl, NAN, {NAN, NAN});
...
TEST_CASE("Integrated Green's tensor works properly", "[GreensTensorPlate]") {
  ...
  SECTION("Integral over Green_fancy_I_temp obeys the crossing relation") {
    ...
    opts.fancy_I = true;
    Greens.integrate_1d_k(Greens_rhs1, opts);
    opts.fancy_I = false;
    opts.fancy_I_temp = true;
    Greens.integrate_1d_k(Greens_rhs2, opts);
...
TEST_CASE("Integrated Green's tensor matches asymptotes",
  ...
  SECTION("Low-frequency asymptote of fancy_I") {
    ...
    PermittivityDrude perm(gamma, omega_p);
    ReflectionCoefficientsLocBulk refl(&perm); 
    GreensTensorPlate Greens(v, za, NAN, &refl, delta_cut, rel_err);
    struct Options_GreensTensor opts;
    opts.class_pt = &Greens;
    ...
    opts.fancy_I = true;
    opts.omega = omega;
    Greens.integrate_1d_k(GT_Num, opts);
  ...
  SECTION("Low-frequency and high temperature asymptote of fancy_I_temp") {
    ...
    opts.fancy_I_temp = true;
    opts.omega = omega;
    Greens.integrate_1d_k(GT_lhs, opts);
