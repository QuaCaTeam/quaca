...
public:
  PolarizabilityBath(double omega_a, double alpha_zero, std::shared_ptr<MemoryKernel> mu,
  ...
  std::complex<double> get_mu(double omega) const { return this->mu->calculate(omega); };
};
